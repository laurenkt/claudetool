...
| `prefer-terraform-chdir` | PreToolUse | `Bash` | Blocks `cd <dir> && terraform … >log` (a cd combined with an output redirect trips Claude Code's "cd with an output redirect" gate); points at terraform's own `-chdir=<dir>` flag, which removes the cd so the redirect is harmless and the command can match a `terraform` allowlist. Only fires when every non-cd segment is terraform (or an inert passenger like echo) and a redirect is present |
